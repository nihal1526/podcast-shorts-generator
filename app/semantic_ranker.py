...
def _call_llm(
    ...
    """
    Unified LLM caller — routes to the provider set in RANKING_PROVIDER (.env).
    Supported: "gemini" (Google Flash - default) | "openai" (GPT-4o-mini) | "ollama"
    """
    active_prov = (provider or os.environ.get("RANKING_PROVIDER") or RANKING_PROVIDER or "gemini").lower().strip()
    if active_prov == "openai" or (os.environ.get("OPENAI_API_KEY") and not os.environ.get("GOOGLE_API_KEY")):
